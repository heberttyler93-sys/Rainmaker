...
def _validate_lengths(predictions, outcomes, name=""):
    if len(predictions) != len(outcomes):
        raise ValueError(
            f"{name}: predictions and outcomes must have the same length, "
...
class ScoringRules:
    ...
    def brier_score(predictions, outcomes):
        _validate_lengths(predictions, outcomes, "brier_score")
    ...
    def log_loss(predictions, outcomes, eps=1e-10):
        _validate_lengths(predictions, outcomes, "log_loss")
    ...
    def calibration_curve(predictions, outcomes, n_bins=5):
        _validate_lengths(predictions, outcomes, "calibration_curve")
        p = np.array(predictions)
        o = np.array(outcomes)
